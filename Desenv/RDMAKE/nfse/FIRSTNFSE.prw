...
Local _cNotaRJ := SF2->F2_NFELETR // ALTERADO DEVIDO A CONFIGURAÇÃO DO SITE DA PREFEITURA DO RIO DE JANEIRO
...
If AllTrim(_cCCM) == "02582880" // Rio de Janeiro  
	// Lpassos 20121213 Ajusta código confirmação que no link não possui o hifen, muda de xxxx-xxxx para xxxxxxxx
//	_cCod := left(_cCod,4)+right(_cCod,4)
	_cCod := StrTran(AllTrim(_cCod),"-","")
	// Problemas no reconhecimento do parametro
	_cLnk = "https://notacarioca.rio.gov.br/nfse.aspx?"
	_cLnk += AllTrim("ccm=" + AllTrim(_cCCM) + "&nf=" + AllTrim(_cNotaRJ) + "&cod=" + AllTrim(_cCod)) 

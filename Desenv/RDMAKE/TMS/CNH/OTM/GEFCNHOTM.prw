...
If ! DF0->(dbSeek(xFilial('DF0')+Str(_nPlanoId,TamSX3('DF0_PLANID')[1],0))) // OVL->INBOUND - FVL->OUTBOUND
	...
	If FreeForUse('DF0',_cNumAge) .and. MayIUseCod(_cMyUseCode)
		...
		Begin Transaction
			...
			ElseIf _cUO == 'FVL'
				_cTipoVe  := AllTrim(_oXML:_CAB_204:_COD_EQUIP:TEXT)
				_cDesVe   := fBuscaTipoVE(_cTipoVE)  // Busca na SX5
			EndIf
...
BeginSql Alias TRAB
	...
	SELECT DISTINCT F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, A1_CGC, A1_NOME, F2_EMISSAO, C5_MENNOTA, C5_NOTA, C5_NUM,
	       F2_VALBRUT, F2_VALICM, F2_VALISS, F2_FILIAL, C5_CCUSTO, SF2.R_E_C_N_O_ AS SF2_RECNO
...
While !(TRAB)->(Eof())
	...
	_aSF2 	:= {F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, A1_CGC, A1_NOME, F2_EMISSAO, F2_VALBRUT, F2_VALICM, F2_VALISS, F2_FILIAL}
...
* para dividir o KM vindo no XML                           *
*----------------------------------------------------------*/
Static Function fBuscaTipoVE(_cTipoVei)
Local _cDesVei := ''
Local _aArea 	 := GetArea()
...
If dbSeek(xFilial('SX5')+'ZB'+'FVL01')
	While !SX5->(Eof())

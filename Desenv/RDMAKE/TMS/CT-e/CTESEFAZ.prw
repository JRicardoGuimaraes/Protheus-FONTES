			...
			If (lTMSCTe) .Or. (AModNot(SF2->F2_ESPECIE)=="57") .Or. (FunName()$"TMSA200" .Or. FunName()$"TMSA500")
				...
				If (!(cAliasDT6)->(Eof())) // (01)
					...
					While !(cAliasDT6)->(Eof()) // (02)
						...
						xINS_DEV := SA1->A1_INSCR				// Inscrição do Tomador do Servico
						// Fim Merge
						dbSelectArea("SF3")
						SF3->(DbSetOrder(4))
						...
						*/
						_cCliMAN  := AllTrim(GETNewPar("MV_XCTEMAN","04858701|")) // Merge em 07/08/2015 - MAM Strack

						If  ( AllTrim( (cAliasDT6)->( DT6_CLIDEV+DT6_LOJDEV ) ) $ _cCliMAN )
...
// Fim da Regra da Gefco para FVL
						...
						If cGerDup == "S"
							...
							If	!AllTrim((cAliasDT6)->DT6_DOCTMS) == StrZero( 8, Len(DT6->DT6_DOCTMS))
								...
								While !(cAliasTagI)->(Eof())
									...
									Else
										cString += '<vComp>' + ConvType((cAliasTagI)->DT8_VALTOT, 15, 2) + '</vComp>'   // Andre
									EndIf
									cString +='</Comp>'
									(cAliasTagI)->(dbSkip())
								EndDo
								*/
								...
								//								nValImp += ( (cAliasTagI)->DT8_VALTOT - (cAliasTagI)->DT8_VALPAS )
						...
						If lFecp
							cQuery += ", F4_DIFAL, CD2_BC, CD2_PFCP,CD2_ALIQ, CD2_ADIF, CD2_PDDES,CD2_VFCP, CD2_VDDES, CD2_VLTRIB " + CRLF
						EndIf
						cQuery += "   FROM " + RetSqlName("SD2") + " D2," + RetSqlName("SF4") + " F4,"  + RetSqlName("CD2") + " CD2 "  + CRLF
						cQuery += "  WHERE D2.D2_FILIAL  = '" + xFilial('SD2') + "'" + CRLF
						If AllTrim((cAliasDT6)->DT6_DOCTMS) == "M"
						...
						While !(cAliasTagJ)->(Eof())
							...
							If lFecp
								If (cAliasRBC)->F4_DIFAL == "1" .And. (cAliasRBC)->F4_COMPL == "S" .And. (cAliasRBC)->F4_CONSUMO == "S"
									nBCICMS := (cAliasRBC)->CD2_BC
									nPERFCP := (cAliasRBC)->CD2_PFCP
									nALQTER := (cAliasRBC)->CD2_ALIQ
									nALQINT := (cAliasRBC)->CD2_ADIF
									nPEDDES := (cAliasRBC)->CD2_PDDES
									nVALFCP := (cAliasRBC)->CD2_VFCP
									nVALDES := (cAliasRBC)->CD2_VDDES
									nVLTRIB := (cAliasRBC)->CD2_VLTRIB
								EndIf
							EndIf
					...
					If (!(cAliasDT6)->(Eof())) // (01)
						...
						While !(cAliasDT6)->(Eof()) // (02)
							...
							If lFecp
								cQuery += ", F4_DIFAL, CD2_BC, CD2_PFCP,CD2_ALIQ, CD2_ADIF, CD2_PDDES,CD2_VFCP, CD2_VDDES, CD2_VLTRIB " + CRLF
							EndIf
							cQuery += "   FROM " + RetSqlName("SD1") + " D1," + RetSqlName("SF4") + " F4 "  + RetSqlName("CD2") + " CD2 "  + CRLF

...
Static Function RTMSR27
	...
	Local oFont30N		:= TFont():New("Times New Roman",,30,,.T.,,,,.T.,.F.)  // Merge
	Local lControl		:= .F.
	Local cAliasInfNF	:= ''
	Local cAliasSeg		:= ''
	Local cPerg			:= 'RTMSR27'
...
//Inicio Merge
		If oNfe:_CTE:_INFCTE:_IDE:_TPAMB:TEXT == "2" .And. Empty((cAliasCT)->DT6_CHVCTG)
			oDacte:Say(0815, 0010, "DOCUMENTO GERADO EM AMBIENTE DE HOMOLOGAÇÃO - SEM VALOR FISCAL", oFont30N, 1400, CLR_HRED,0305)
		EndIf
//Fim Merge
...
Static Function TMSGetComp( oObject )
	...
	// Inicio Merge
	// Customização para a separação dos Impostos
	/*
	If lElectrolux
		aResult := {}
...
Data	  		- 13/04/2015
Autor	  		- André Costa
Descricao	- Faz o calculo do Imposto para ser destacado no DACTE
*/

Static Function fCalculaImposto( oObject ,pValor )
	Local _nValor		:= Val( pValor )
	Local _Cliente		:= ( cAliasCT )->( DT6_CLIDEV + DT6_LOJDEV )
	Local _MV_ELETRO	:= GetNewPar("MV_XELETRO","")
	Local _nICMS		:= 0
	Local _nValImp		:= 0

	Begin Sequence

		If ! ( _Cliente $ _MV_ELETRO )
			Break
		EndIf

		If Valtype(XmlChildEx(oObject:_CTE:_INFCTE:_IMP:_ICMS,'_ICMS00'))  == 'U'
			Break
		EndIf

		_nICMS	:= Val( oObject:_CTE:_INFCTE:_IMP:_ICMS:_ICMS00:_PICMS:TEXT ) / 100
		_nValImp	:= ( _nValor * _nICMS )

		If Len( aImposto ) == 0
			aadd( aImposto , _nValImp )
		Else
			aImposto[ Len( aImposto ) ] += _nValImp
		EndIf
...
Static Function fBuscaObsVia()
	...
	If DbSeek(xFilial("DTQ")+(cAliasCT)->DT6_DOC+(cAliasCT)->DT6_SERIE)
			...
			If ZA6->( dbSeek( DTQ->DTQ_AS, .T.  ) ) .and. !Empty(ZA6->ZA6_OBSVIA)	// Unificação de fontes do GPO - Ricardo em 16/02/2015
				_cObs := Alltrim( ZA6->ZA6_OBSVIA )
			EndIf
		EndIf
	EndIf
